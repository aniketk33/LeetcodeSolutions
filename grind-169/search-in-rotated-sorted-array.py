...
def search(self, num_arr, target) -> int:
    n = len(num_arr)
    left, right = 0, n - 1
    min_index = -1
    ...
    # search in the sub-array
    if target >= num_arr[0] and target > num_arr[min_index]:
        target_index = self.binary_search(num_arr, left=0, right=((min_index + n) - 1) % n, target=target)
    else:
        target_index = self.binary_search(num_arr, left=min_index, right=n - 1, target=target)
